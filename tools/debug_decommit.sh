#!/usr/bin/env bash
# Test with SMASH_NO_DECOMMIT=1 to see if madvise/decommit is the issue
set -euo pipefail
...
OUT=$NCC/smash/build/debug_decommit_$(date +%Y%m%d_%H%M%S)
RUNS=${1:-3}
...
echo "=== Smash Decommit Debug Test ==="
echo "Running $RUNS iterations per config"
echo ""

# Config 1: NO_DECOMMIT=1 (no madvise, keep page contents)
echo "no_decommit (SMASH_NO_DECOMMIT=1):"
for i in $(seq 1 $RUNS); do
  run_compile "no_decommit" "$i" "SMASH_VM_GIB=48 SMASH_COLD_TIMEOUT_SEC=10 SMASH_NO_DECOMMIT=1 SMASH_DEFER_MADVISE=1"
done

# Config 2: FIXAV=1 (proper ordering: PROT_NONE -> membarrier -> madvise)  
echo ""
...
# Config 3: Default full mode (for reference)
echo ""
echo "default:"
for i in $(seq 1 $RUNS); do
  run_compile "default" "$i" "SMASH_VM_GIB=48 SMASH_COLD_TIMEOUT_SEC=10 SMASH_DEFER_MADVISE=1"
done
...
echo ""
echo "Pass rates:"
for cfg in no_decommit fixav default; do
