...
class ThreadCache {
    ...
    //
    // Depth per lane = kThreadCacheMaxPerClass / kCacheLanes (= 64 with defaults).
    // Total = kCacheLanes * kNumClasses * 64 * 8 = 4 * 36 * 64 * 8 = 72 KB
    // (same as the old flat kNumClasses * 256 * 8 = 72 KB).
    static constexpr int kCacheLanes = 4;
    static constexpr int kPerLaneDepth = kThreadCacheMaxPerClass / kCacheLanes;
    static_assert(kPerLaneDepth >= 16, "per-lane cache too shallow");
