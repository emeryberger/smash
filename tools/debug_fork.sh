#!/usr/bin/env bash
# Test with compression pausing across fork boundary
set -euo pipefail
...
OUT=$NCC/smash/build/debug_fork_$(date +%Y%m%d_%H%M%S)
RUNS=${1:-3}
...
echo "=== Smash Fork Debug Test ==="
echo "Running $RUNS iterations per config"
echo ""

# Config 1: Deferred phases - skip compression for first 30s in subprocesses
echo "deferred_30s (SMASH_DEFER_PHASES_MS=30000):"
for i in $(seq 1 $RUNS); do
  run_compile "deferred_30s" "$i" "SMASH_VM_GIB=48 SMASH_COLD_TIMEOUT_SEC=10 SMASH_DEFER_PHASES_MS=30000 SMASH_DEFER_MADVISE=1"
done

# Config 2: FIXAV + SNAPSHOT_VERIFY (most conservative ordering)
echo ""
echo "fixav_verify (SMASH_FIXAV=1 SMASH_SNAPSHOT_VERIFY=1):"
for i in $(seq 1 $RUNS); do
  run_compile "fixav_verify" "$i" "SMASH_VM_GIB=48 SMASH_COLD_TIMEOUT_SEC=10 SMASH_FIXAV=1 SMASH_SNAPSHOT_VERIFY=1"
done

# Config 3: Longer cold timeout (less aggressive compression)
echo ""
echo "cold_30s (SMASH_COLD_TIMEOUT_SEC=30):"
for i in $(seq 1 $RUNS); do
  run_compile "cold_30s" "$i" "SMASH_VM_GIB=48 SMASH_COLD_TIMEOUT_SEC=30 SMASH_DEFER_MADVISE=1"
done
...
echo ""
echo "Pass rates:"
for cfg in deferred_30s fixav_verify cold_30s; do
