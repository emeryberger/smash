...
- `SMASH_CPU_PRESSURE_PAUSE=1`: Default ON. Adaptive compression pause for CPU-saturated hosts. When the app's own threads leave no spare cores (the worker cap collapses to 1) for a few consecutive ticks, the compressor pauses Phase 2 (compress) + Phase 3 (monitor) **and skips the per-tick page scan** — resuming automatically within a tick once cores free up (e.g. between passes / at teardown). On such workloads compression is net-useless churn (pages compress then immediately fault back in) while the RSS win comes from large-alloc management + madvise, not compression. Measured on neuron-cc `test7_full` (192-core box, ~200-thread walrus): large-only wall regression **+50.7% → +19.6%** vs glibc (61% of the regression recovered) with the peak-RSS win preserved (−13.4% → −12.2%) and AUC-RSS improved from +36% to +7%. Set `=0` to disable (ablation / latency-critical workloads that want compression regardless of CPU pressure). Gated by `SMASH_CPU_PRESSURE_CAP` (which computes the underlying cap); if that is disabled this is a no-op.
