#!/usr/bin/env bash
# Benchmark smash on neuron-cc test7_full compilation
# Phase 1: Profile run (collect compression statistics)
...
OUT=$NCC/smash/build/smash_bench_$(date +%Y%m%d_%H%M%S)
...
run_compile() {
  ...
  # Build env string - need to eval to handle spaces in extra_env
...
# Smash LARGE_ONLY mode - profile run
echo ""
echo "Phase 1: Smash profile run (LARGE_ONLY mode)"
run_compile "smash_profile" "$SMASH" \
  "SMASH_LARGE_ONLY=1 SMASH_COLD_TIMEOUT_SEC=5 SMASH_PROFILE_FILE=$PROFILE SMASH_PROFILE_FILE_RW=1"
...
# Smash LARGE_ONLY mode - using profile
echo ""
echo "Phase 2: Smash with profile (LARGE_ONLY mode)"
run_compile "smash_with_profile" "$SMASH" \
  "SMASH_LARGE_ONLY=1 SMASH_COLD_TIMEOUT_SEC=5 SMASH_PROFILE_FILE=$PROFILE"

# Smash LARGE_ONLY mode - shorter cold timeout
echo ""
echo "Phase 3: Smash aggressive (1s cold timeout)"
run_compile "smash_aggressive" "$SMASH" \
  "SMASH_LARGE_ONLY=1 SMASH_COLD_TIMEOUT_SEC=1 SMASH_PROFILE_FILE=$PROFILE"
