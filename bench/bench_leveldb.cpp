// bench_leveldb.cpp - LevelDB benchmark
//
// Populates a LevelDB database, then does hot reads (recent keys) while
// older data goes cold. Measures RSS at each phase.
//
// Build: link against -lleveldb
// Usage: bench_leveldb --db /tmp/testdb --keys 500000 --value-size 512 --cool 10 --serve 20
...
int main(int argc, char* argv[]) {
    std::string db_path = "/tmp/bench_leveldb_test";
    int num_keys = 500000;
    int value_size = 512;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "--cache") == 0 && i + 1 < argc) cache_mb = atoi(argv[++i]);
        else if (strcmp(argv[i], "--quick") == 0) quick = true;
    }
    ...
    fprintf(stderr, "=== LevelDB Benchmark ===\n");
    fprintf(stderr, "Keys: %d, Value size: %d, Cool: %ds, Serve: %ds, Cache: %dMB\n\n",
            num_keys, value_size, cool_duration, serve_duration, cache_mb);

    leveldb::Options options;
    options.create_if_missing = true;
    options.write_buffer_size = 64 * 1024 * 1024;
    options.block_cache = leveldb::NewLRUCache(cache_mb * 1024 * 1024);
    ...
    fprintf(stderr, "Warming cache with full scan...\n");
    {
    ...
    double reduction_pct = 100.0 * (peak_rss - min_rss) / peak_rss;
    printf("METRIC reduction_pct %.1f\n", reduction_pct);

    // Compute AUC (area under RSS curve, in MB-seconds)
    // RSS samples are approximately 1 second apart during cool+serve phases
    double auc_mb_sec = 0.0;
    for (const auto& sample : g_rss_timeline) {
        auc_mb_sec += sample.rss_mb;
    }
    printf("METRIC auc_mb_sec %.1f\n", auc_mb_sec);

    if (const char* rss_dir = std::getenv("SMASH_RSS_DIR")) {
        dumpRssTimeline((std::string(rss_dir) + "/leveldb_rss.csv").c_str());
    }

    if (const char* lat_dir = std::getenv("SMASH_LATENCY_DIR")) {
        std::string path = std::string(lat_dir) + "/leveldb_cold.csv";
        ...
        std::string hot_path = std::string(lat_dir) + "/leveldb_hot.csv";
